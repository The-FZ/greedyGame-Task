...
let EnhancedTable = props => {

  function createData(timestamp, game, revenue, impressions, eCPM) {
    return { timestamp, game, revenue, impressions, eCPM };
  }

  const rows = props.dataToShow.map(a => {
    let eCPM = (a.revenue / a.impressions) * 1000;
    eCPM = eCPM.toFixed(2);
    return createData(a.timestamp, a.game, a.revenue, a.impressions, eCPM);
  })
